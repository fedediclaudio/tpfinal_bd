...
public abstract class ChangeStatusCommand {
    ...
    protected Order getOrder(ChangeOrderStatusDto request) throws PersistenceEntityException {
        return orderRepository.findById(IdConvertionHelper.convert(request.getOrderId()))
                .orElseThrow(() -> new PersistenceEntityException("Can't find order with id "+request.getOrderId()));
    }
}
